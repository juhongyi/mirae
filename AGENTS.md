...
Local harness (pnpm, runs on the host): `pnpm install` registers the lefthook pre-commit hook and installs the pinned SDK deps (`@n8n/workflow-sdk`/`n8n-nodes-base` versions match the n8n image). `pnpm validate` runs workflow-sdk validation over `n8n/workflows/*.ts`; `pnpm lint`/`pnpm check` run Biome. The pre-commit hook runs `biome check --write` then `pnpm validate`.
