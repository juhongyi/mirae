...
기여자 콘텐츠의 기계적 사전 검증, 심사 대기열 관리, SLA 초과 감지, 심사 결정 및 알림(Phase 1)과 정산 원천 데이터 제공(Phase 2)을 담당하는 스텁 API다. 정산 이상치 판정과 명세 생성은 API가 아닌 워크플로우가 수행한다. Phase 3 SVG 변환은 n8n Code 노드가 직접 수행하며, API는 변환 실패 알림과 후속 제출 처리를 담당한다. 창작성과 품질에 대한 판단은 자동화하지 않으며, 상태는 프로세스 메모리에 저장된다.
...
| `POST` | `/notifications` | 반려, SLA 초과, 이상치 고지, 명세 배포, 변환 실패 결과를 템플릿 메시지로 생성한다. |
| `GET` | `/notifications` | 생성된 알림을 조회한다. |
| `GET` | `/audit-events` | 제출, 검증, 대기열, 심사, 알림 처리 이력을 조회한다. |

## Phase 2: 정산·통보

정산 영역에서 API는 **정산 원천 데이터를 제공**할 뿐이며, 이상치 판정과 명세 생성은 워크플로우의 책임이다.
...
정산 이상치 고지와 명세 배포는 워크플로우가 판정·생성한 결과를 알림으로 전달한다. `kind`는 `anomaly_alert`(이상치 고지) 또는 `settlement_statement`(명세 배포)이며, `detail`에 구체적인 사유 또는 명세 본문을 담는다.
